...
-- LibTimer is currently limited to firing timers at a frequency of 0.1s. This constant may change
...
if not LibTimer then return end -- No upgrade needed

LibTimer.hash = LibTimer.hash or {}         -- Array of [0..BUCKET-1] = linked list of timers (using .next member)
                                            -- Linked list gets around ACE-88 and ACE-90.
LibTimer.selfs = LibTimer.selfs or {}       -- Array of [self]={[handle]=timerobj, [handle2]=timerobj2, ...}
...
LibTimer.frame:SetScript("OnUpdate", OnUpdate)
LibTimer.frame:SetScript("OnEvent", OnEvent)
LibTimer.frame:RegisterEvent("PLAYER_REGEN_ENABLED")
