...
-- LibTimer supports one-shot timers and repeating tiemrs.  LibTimer is
...
-- LibTimer can fire timers at pretty much any resolution that the game can handle.
...
-- without having to explcitly calling LibTimer itself.\\
-- It is recommended to embed LibTimer, otherwise you'll have to specify a custom `self`
-- on calls you make into LibTimer
-- @class file
-- @name LibTimer-1.0
...
LibTimer.selfs = LibTimer.selfs or {}
LibTimer.frame = LibTimer.frame or CreateFrame("Frame", "LibTimerFrame")
...
do
	...
	function OnFinished(self, elapsed)
		...
		if not repeating or canceled_in_OnFinished then
			LibTimer.CancelTimer(self.object, tostring(self), true)
...
function LibTimer:TimeLeft(handle)
	if not handle then return end
	if type(handle) ~= "string" then
...
-- LibTimer:OnEmbedDisable( target )
-- target (object) - target object that LibTimer is embedded in.
--
...
for addon in pairs(LibTimer.embeds) do
	LibTimer:Embed(addon)
end
