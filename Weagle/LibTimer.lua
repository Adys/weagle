--- **LibTimer-1.0** provides a centeral facility for registering timers.
-- LibTimer supports one-shot timers and repeating tiemrs.  Timer is
...
-- Timer can fire timers at pretty much any resolution that the game can handle.
...
--
-- **LibTimer-1.0** can be embeded into your addon, either explicitly by calling
-- LibTimer:Embed(MyAddon) or by specifying it as an embeded library in your AceAddon.
-- All functions will be availalbe on your addon object and can be accesssed directly,
-- without having to explcitly calling Timer itself.\\
-- It is recommended to embed Timer, otherwise you'll have to specify a custom `self`
-- on calls you make into Timer
-- @class file
-- @name LibTimer-1.0
...
Weagle.LibTimer = {}
local Timer = Weagle.LibTimer

local MAJOR, MINOR = "LibTimer-1.0", 2

Timer.selfs = {}
Timer.frame = CreateFrame("Frame", "Timer10Frame")
...
local function new()
	...
	if timer then
		...
		local ag = Timer.frame:CreateAnimationGroup()
...
do
	...
	function Timer:CancelTimer(handle, silent)
		if not handle then return end
		if type(handle) ~= "string" then
			error(MAJOR..": CancelTimer(handle): 'handle' - expected a string", 2)
		end
		local selftimers = Timer.selfs[self]
	...
	function OnFinished(self, elapsed)
		...
		if not repeating or canceled_in_OnFinished then
			Timer.CancelTimer(self.object, tostring(self), true)
...
-- end
function Timer:ScheduleTimer(callback, delay, arg)
...
--   end
-- end
function Timer:ScheduleRepeatingTimer(callback, delay, arg)
	return Reg(self, callback, delay, arg, true)
end

--- Cancels all timers registered to the current addon object ('self')
function Timer:CancelAllTimers()
	...
	if selftimers then
		for handle,v in pairs(selftimers) do
			Timer.CancelTimer(self, handle, true)
		end
	end
end
...
function Timer:TimeLeft(handle)
	if not handle then return end
	if type(handle) ~= "string" then
...
-- Timer:OnEmbedDisable( target )
-- target (object) - target object that Timer is embedded in.
--
-- cancel all timers registered for the object
function Timer:OnEmbedDisable( target )
	target:CancelAllTimers()
end


for addon in pairs(Timer.embeds) do
	Timer:Embed(addon)
end
